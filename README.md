...
3. `Alt+Tab` back to the Sonar page. Copy each issue's link (on Firefox, right click the blue titles and press `L`, scrolling down as needed) and wait for subfolders to appear in the script folder. Currently, each issue should be represented with a long named subfolder with the below contents. why.md and how.md are hidden by .gitignore to avoid redistributing SonarSource content.
...
## Planned features
- Add educational links to where.json 
