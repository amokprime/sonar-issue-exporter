...
- For multi-file sessions (e.g. the v1.0.0 refactor delivering `sie.py` + `sie-migrate.sh` + `README.md` + `tests/` + `ai/chat.z.ai/`), zip all deliverables into a single `download/sonar-issue-exporter-1.0.0.zip`. The user unzips locally. Keep the loose files in `download/` too — the zip is for convenience, the loose files are for immediate inspection.
...
For sonar-issue-exporter specifically: when context compacts and you lose track of the v1.0.0 refactor details (single-file script, `--migrate` flag, no clipboard watcher, etc.), re-read `MEMORY.md` first — it's the leanest summary and explicitly flags what training data shadows (e.g. "training data may suggest `uv tool install` — for `sie` 1.0.0+, the install is a single `install -Dm755 sie.py ~/.local/bin/sie`").